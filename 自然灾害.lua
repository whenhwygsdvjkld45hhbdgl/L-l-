...
local Main = Window:Tab({Title = "主要功能", Icon = "settings"})
        ...
        if Value then
            killThread = task.spawn(function()
                while true do
                    ...
                    for playerName, attackTime in pairs(attackedPlayers) do
                        if currentTime - attackTime >= 60 then
                            attackedPlayers[playerName] = nil
                        end
                    end
                        ...
                        if localChar and localChar:FindFirstChild("HumanoidRootPart") then
                            ...
                            for _, player in ipairs(players) do
                                if player ~= localPlayer then
                                    ...
                                    if not attackedPlayers[playerName] then
                                        ...
                                        if char then
                                            ...
                                            if humanoid and rootPart then
                                                ...
                                                if distance <= attackRange then
                                                    ...
                                                    humanoid.Health = 0
                                                    attackedPlayers[playerName] = tick()
            ...
            orbitThread = task.spawn(function()
                while true do
                    ...
                    pcall(function()
                        ...
                        if localChar and localChar:FindFirstChild("HumanoidRootPart") then
                            local localPos = localChar.HumanoidRootPart.Position
                            local centerPos = Vector3.new(localPos.X, localPos.Y + orbitHeight, localPos.Z)
                            
                        
                            for _, obj in ipairs(workspace:GetDescendants()) do
                                if obj:IsA("BasePart") and not obj.Anchored and not orbitingObjects[obj] then
                                    local distance = (obj.Position - centerPos).Magnitude
                                    
                                    if distance <= orbitRange then
                                        orbitingObjects[obj] = {
                                            originalPosition = obj.Position,
                                            angle = math.random() * 2 * math.pi,
                                            distance = 5 + math.random() * (orbitRange - 5),
                                            speed = orbitSpeed * (0.5 + math.random())
                                        }
                                        obj.Anchored = true
                                    end
                                end
                            ...
                            local time = tick()
                            for obj, orbitData in pairs(orbitingObjects) do
                                if obj and obj.Parent then
                                 
                                    local angle = orbitData.angle + time * orbitData.speed
                                    local x = math.cos(angle) * orbitData.distance
                                    local z = math.sin(angle) * orbitData.distance
                                    
                                    local orbitPos = centerPos + Vector3.new(x, 0, z)
                                    
                                
                                    obj.Position = orbitPos
                                    
                                
                                    obj.CFrame = CFrame.lookAt(orbitPos, centerPos)
                                else
                                    orbitingObjects[obj] = nil
                                end
                            end
                        end
                    end)
...
local k = Window:Tab({Title = "人物", Icon = "settings"})
k:Toggle({
    Title = "锁血",
    Value = false,
    Callback = function(state)
        ...
        if state then
            gm.__newindex = function(t, k, v)
                if k == "Health" and t:IsA("Humanoid") and t.Parent == Character then
                    return old(t, k, 100)
                end
                return old(t, k, v)
            end
