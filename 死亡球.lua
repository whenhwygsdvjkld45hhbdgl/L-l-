...
local SettingsSection = Window:Tab({Title = "自动拦截", Icon = "target"})
local _G = _G or {}
_G.AutoInterceptEnabled = false
local ball = nil
local fieldPart = nil
...
_G.AutoInterceptEnabled = Value
if Value then
WindUI:Notify("自动拦截", "自动拦截已开启", 3)
startAutoIntercept()
else
WindUI:Notify("自动拦截", "自动拦截已关闭", 3)
stopAutoIntercept()
end
end
})
...
Tooltip = "设置自动拦截的触发距离",
Callback = function(Value)
_G.InterceptDistance = Value
if _G.AutoInterceptEnabled then
WindUI:Notify("拦截设置", "拦截距离已更新: " .. Value, 2)
end
end
})
SettingsSection:Toggle({
...
_G.ShowField = Value
if fieldPart then
fieldPart.Transparency = Value and 0.5 or 1
end
WindUI:Notify("视觉设置", Value and "力场范围显示已开启" or "力场范围显示已关闭", 3)
end
})
function startAutoIntercept()
interceptCount = 0
...
local ballConnection = workspace.ChildAdded:Connect(function(v)
if v.Name == "Part" then
ball = v
WindUI:Notify("目标更新", "发现新球体", 2)
end
end)
if _G.ShowField ~= false then
...
if not _G.AutoInterceptEnabled then
return
end
...
if highlight and highlight.FillColor == Color3.new(1, 0, 0) and distance <= (_G.InterceptDistance or 10) then
VirtualInputService:SendKeyEvent(true, Enum.KeyCode.F, false, nil)
wait(0.1)
VirtualInputService:SendKeyEvent(false, Enum.KeyCode.F, false, nil)
interceptCount = interceptCount + 1
WindUI:Notify("拦截成功", "成功拦截球体! 次数: " .. interceptCount, 2)
end
updateStatusDisplay("运行中", "已找到", interceptCount)
else
updateStatusDisplay("运行中", "未找到", interceptCount)
end
end)
getgenv().BallConnection = ballConnection
getgenv().InterceptConnection = connection
updateStatusDisplay("已启动", ball and "已找到" or "未找到", interceptCount)
end
function stopAutoIntercept()
if getgenv().BallConnection then
getgenv().BallConnection:Disconnect()
getgenv().BallConnection = nil
end
if getgenv().InterceptConnection then
getgenv().InterceptConnection:Disconnect()
getgenv().InterceptConnection = nil
end
if fieldPart then
fieldPart:Destroy()
fieldPart = nil
...
ball = nil
updateStatusDisplay("已停止", "未找到", interceptCount)
end
function updateStatusDisplay(status, ballStatus, count)
if status == "已启动" then
WindUI:Notify("系统状态", "自动拦截系统已启动", 2)
elseif status == "已停止" then
WindUI:Notify("系统状态", "自动拦截系统已停止", 2)
end
end
