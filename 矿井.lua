...
Window.Open = function(...)
    ...
    -- Tabs --
    local MineTab = Window:Tab({Title = "挖矿", Icon = "settings"})
    local TeleportTab = Window:Tab({Title = "传送", Icon = "map-pin"})
    local ShopTab = Window:Tab({Title = "商店", Icon = "shopping-cart"})
    local MiscTab = Window:Tab({Title = "其他", Icon = "more-horizontal"})

    -- Mining Tab --
    MineTab:Section({Title = "自动挖矿功能"})
    ...
    MineTab:Dropdown({
        ...
        Callback = function(Value)
            if Value == "最大" then
                MiningStrength = 1
            elseif Value == "良好" then
                MiningStrength = 0.8
            elseif Value == "一般" then
                MiningStrength = 0.7
            elseif Value == "较差" then
                MiningStrength = 0.6
            end
            if AutoMine then
                if MiningThread then
    ...
    MineTab:Dropdown({
        Title = "收集速度",
        Default = "慢速",
        Values = {"瞬间 (可能延迟)", "快速", "慢速"},
        Callback = function(Value)
            if Value == "瞬间 (可能延迟)" then
                CollectSpeed = 0
            elseif Value == "快速" then
                CollectSpeed = 0.125
            elseif Value == "慢速" then
                CollectSpeed = 0.5
            end
        end
    })
    ...
    -- Teleport Tab --
    TeleportTab:Section({Title = "地点传送"})
    ...
    TeleportTab:Section({Title = "自定义传送"})
    ...
    ShopTab:Dropdown({
        ...
        Values = {
            ...
            "大司马脚本bite Pickaxe ($6,000,000)",
            "Sun大司马脚本 Pickaxe ($7,500,000)",
            "Rose大司马脚本 Pickaxe ($9,000,000)",
            "Shadow大司马脚本 Pickaxe ($12,500,000)"
        }, 
        Callback = function(Value)
    ...
    ShopTab:Dropdown({
        ...
        Values = {
            ...
            "大司马脚本bite Radar ($7,000,000)",
            "Sun大司马脚本 Radar ($8,500,000)",
            "Rose大司马脚本 Radar ($10,000,000)",
            "Shadow大司马脚本 Radar ($13,000,000)"
        }, 
        Callback = function(Value)
            BuyItem:FireServer(string.gsub(Value, "%s%(%$[%d,]+%)", ""))
        end
        ...
        Callback = function(bool)
            if bool then
                if fireproximityprompt then
                    ...
                    WindUI:Notify({
                        Title = "不兼容的注入器",
                        Content = "您的注入器与fireproximityprompt不兼容",
                        Duration = 6
                    })
                end
            else
