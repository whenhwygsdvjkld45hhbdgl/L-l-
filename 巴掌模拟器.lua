...
local HomeTab = Window:Tab({Title = "主要功能", Icon = "home"})
        ...
        if state then
            ...
            task.spawn(function()
                while farming do
                    task.wait()
                    pcall(function()
                        for _, Human in next, game.Players:GetPlayers() do
                            if Human ~= game.Players.LocalPlayer and Human.Character and not Human.Character:FindFirstChild("isParticipating") and Human.Character:FindFirstChild("Torso") and Human.Character:FindFirstChild("Head") and Human.Character:FindFirstChild("entered") and Human.Character.Head:FindFirstChild("UnoReverseCard") == nil and Human.Character:FindFirstChild("rock") == nil and Human.Character.Ragdolled.Value == false then
                                if game.Players.LocalPlayer.Character:FindFirstChild("entered") and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then                             
                                    task.wait(0.1)
                                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = Human.Character.RightLeg.CFrame * CFrame.new(6,-5,6)   
                                    task.wait()   
                                    game.Players.LocalPlayer.Character.Humanoid.PlatformStand = true
                                    task.wait(0.25)
                                    shared.gloveHits[getGlove()]:FireServer(Human.Character.Torso)
                                    task.wait(0.25)
                                end
                            end
                        end
            ...
            if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") and game.Players.LocalPlayer.Character.Humanoid.PlatformStand == true then
                task.wait(3)
    ...
    Callback = function()
        if not game:GetService("BadgeService"):UserHasBadgeAsync(game.Players.LocalPlayer.UserId, 2129212145) then
            if game.Players.LocalPlayer.Character:FindFirstChild("entered") == nil then    
                repeat 
                    task.wait(0.5)   
                    firetouchinterest(game.Players.LocalPlayer.Character.Head, workspace.Lobby.Teleport1.TouchInterest.Parent, 0)
                    firetouchinterest(game.Players.LocalPlayer.Character.Head, workspace.Lobby.Teleport1.TouchInterest.Parent, 1)          
...
-- 战斗功能标签页
local CombatTab = Window:Tab({Title = "战斗功能", Icon = "sword"})
    ...
    Callback = function(state)
        ...
        if state then
            task.spawn(function()
                while explode do
                    task.wait(0.01)
                    game:GetService("ReplicatedStorage").rhythmevent:FireServer("AoeExplosion", 86)
                end
            end)
        end
    end
                    ...
                    pcall(function()    
                        for _, Player in next, game.Players:GetPlayers() do
                            if Player ~= game.Players.LocalPlayer and Player.Character and Player.Character:FindFirstChild("entered") then
                                if Player.Character:FindFirstChild("Head") and Player.Character.Head:FindFirstChild("UnoReverseCard") == nil and Player.Character:FindFirstChild("rock") == nil then
                                    if game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                        ...
                                        if Magnitude <= 25 then
                                            shared.gloveHits[getGlove()]:FireServer(Player.Character.Head)
                                        end
                                    end
                                end
                            end
...
-- 其它功能标签页
local OtherTab = Window:Tab({Title = "防护功能", Icon = "shield"})

-- 防护功能
local antiJack = false
OtherTab:Toggle({
    Title = "防Hallow-Jack",
    Default = false,
    Callback = function(state)
...
OtherTab:Toggle({
    ...
    Callback = function(state)
        antiBoost = state
        if state then
            if workspace[game.Players.LocalPlayer.Name]:FindFirstChild("BoosterObject") then
                workspace[game.Players.LocalPlayer.Name].BoosterObject:Destroy()
            end
            workspace[game.Players.LocalPlayer.Name].DescendantAdded:Connect(function(v)
                if antiBoost and v.Name == "BoosterObject" then
                    task.wait(0.1)
                    v:Destroy()
                end
            end)
    ...
    Callback = function(state)
        ...
        if state then
            game.Players.LocalPlayer.PlayerGui.DescendantAdded:Connect(function(UI)
                if antiMail and UI.Name == "MailPopup" then
                    UI.Frame.Visible = false
                    task.wait()
                    if game.Players.LocalPlayer.Character.Head:FindFirstChild("MailIcon") then
        ...
        if state then
            task.spawn(function()
                while antiStun do
                    ...
                    if character and character:FindFirstChild("Humanoid") and character.Humanoid.PlatformStand == true and not farming and not character.Ragdolled.Value == true and workspace:FindFirstChild("Shockwave") then
    ...
    Callback = function(state)
        ...
        if state then
            task.spawn(function()
                while antiSquid do
                    task.wait()
                    for _,v in pairs(game.Players.LocalPlayer.PlayerGui.SquidInk:GetChildren()) do
                        if v.Parent then
                            v:Destroy()
                        end
                    end
...
bypass = hookmetamethod(game, "__namecall", function(method, ...) 
    if getnamecallmethod() == "FireServer" then
        if method == game.ReplicatedStorage.Ban then
            return
        elseif method == game.ReplicatedStorage.AdminGUI then
            return
        elseif method == game.ReplicatedStorage.WalkSpeedChanged then
            return
        end
    end
