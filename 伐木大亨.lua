...
-- 伐木大亨脚本加载函数
local function loadLumberTycoonScript()
    pcall(function()
        loadstring(game:HttpGet('https://api.junkie-development.de/api/v1/luascripts/public/600f928a270ffc5bc686c79f0c69d2c32bfdde1a019227379cdd69114e44b081/download'))()
        WindUI:Notify({
            Title = "加载成功",
            Content = "伐木大亨脚本已启动！",
            Duration = 3,
...
WindUI:Popup({
    Title = "伐木大亨脚本",
    IconThemed = true,
    Content = "欢迎尊贵的用户" .. game.Players.LocalPlayer.Name .. "使用伐木大亨脚本 当前版本型号:V3",
    Buttons = {
        {
            Title = "取消",
...
-- 替换为伐木大亨核心功能面板
local Main = Window:Tab({Title = "核心功能", Icon = "settings"})
Main:Button({
    Title = "启动伐木大亨脚本",
    Icon = "play",
    Callback = function()
        loadLumberTycoonScript()
        playSound()
    end
})
Main:Button({
...
Settings:Button({
    ...
    Callback = function()
        ...
        setclipboard("伐木大亨脚本 设置: " .. game:GetService("HttpService"):JSONEncode(settings))
        playSound()
    end
})
Window:OnClose(function()
